...
Intro sequence:
1) Start prompt (press any key)
2) Title screen: "MID — Monetary Incentive Delay Task"
3) Short instruction screen
Then the task begins.

Run:
    python mid_psychopy_pc_yaml_v3.py --config mid_config.yml
"""
...
def load_config(path):
    ...
    cfg['staircase'].setdefault('max_ms', 900)
    cfg['staircase'].setdefault('step_ms', 10)
    cfg['staircase'].setdefault('per_condition_max_ms', {})

    cfg.setdefault('rscore', {})
    cfg['rscore'].setdefault('enabled', True)
    cfg['rscore'].setdefault('window', 20)
    cfg['rscore'].setdefault('threshold', 75.0)
    cfg['rscore'].setdefault('scale', 0.9)
    cfg['rscore'].setdefault('scope', 'global')
    ...
    cfg['visuals'].setdefault('target_symbol', '◉')
    cfg['visuals'].setdefault('cue_symbols', {"+1": "＋", "-1": "－", "0": "○"})
    cfg['visuals'].setdefault('target_font_height', 0.12)
    cfg['visuals'].setdefault('cue_font_height', 0.12)
    cfg['visuals'].setdefault('text_font_height', 0.06)
...
def main():
    ...
    fixation = visual.TextStim(
        win, text="+", color=cfg['win']['fixation_color'],
    ...
    cue_stim = visual.TextStim(
        win, text="", color=cfg['win']['text_color'],
        height=cfg['visuals']['cue_font_height'], font=cfg['win']['font']
    )
    target_stim = visual.TextStim(
        win, text=cfg['visuals']['target_symbol'], color=cfg['win']['text_color'],
        height=cfg['visuals']['target_font_height'], font=cfg['win']['font']
    )
    ...
    cue_symbol_map = {int(k): v for k, v in cfg['visuals']['cue_symbols'].items()}
    cond_symbol = {label: cue_symbol_map[meta["valence"]] for label, meta in cond_meta.items()}
    ...
    # ------------------------
    # Intro sequence (3 screens)
    # ------------------------

    # 1) Start prompt
    start_prompt = visual.TextStim(
        win,
        text="Bitte Taste drücken, um zu starten.",
    ...
    title_sub = visual.TextStim(
        win,
        text="Weiter mit Taste.",
        color=cfg['win']['text_color'],
        height=cfg['visuals']['text_font_height'],
        font=cfg['win']['font'],
        pos=(0, -0.25)
    )
    title_text.draw(); title_sub.draw()
    win.flip()
    core.wait(0.3)
    event.clearEvents()
    keys = event.waitKeys()
    ...
    instruction_text = visual.TextStim(
        ...
        text=(
            ...
            "Ein Symbol kündigt an, ob Sie einen Gewinn erzielen oder einen Verlust vermeiden können.\n"
            "Wenn der Zielkreis erscheint, drücken Sie so schnell wie möglich die Leertaste.\n"
            "Je schneller Sie reagieren, desto mehr Punkte können Sie gewinnen.\n\n"
    ...
    def run_trial(trial_idx, block_idx, cond_label):
        ...
        cue_stim.draw(); win.flip(); core.wait(cue_ms / 1000.0); check_escape()
        ...
        if cfg['rscore']['enabled']:
            if isinstance(rscore_hist, dict):
                ...
                r = (sum(hist) / len(hist) * 100.0) if len(hist) > 0 else 0.0
                if len(hist) > 0 and r > cfg['rscore']['threshold']:
                    target_ms = max(cfg['staircase']['min_ms'], int(target_ms_pre * cfg['rscore']['scale']))
                else:
                    target_ms = target_ms_pre
                rscore_value = f"{r:.1f}" if len(hist) > 0 else ""
            else:
                r = (sum(rscore_hist) / len(rscore_hist) * 100.0) if len(rscore_hist) > 0 else 0.0
                if len(rscore_hist) > 0 and r > cfg['rscore']['threshold']:
                    target_ms = max(cfg['staircase']['min_ms'], int(target_ms_pre * cfg['rscore']['scale']))
                else:
                    target_ms = target_ms_pre
                rscore_value = f"{r:.1f}" if len(rscore_hist) > 0 else ""
        else:
            target_ms = target_ms_pre
            rscore_value = ""
        ...
        if meta["valence"] > 0:
            ...
            fb = "N E U T R A L"
        if cfg['task']['show_cumulative_points']:
            fb += f"\nPunkte gesamt: {points_total:d}"

        feedback_text.text = fb
        feedback_text.draw(); win.flip(); core.wait(fb_ms / 1000.0); check_escape()
    ...
    if cfg['task']['practice_trials'] > 0:
        ...
        txt.draw(); win.flip()
        core.wait(0.3); event.clearEvents()
        keys = event.waitKeys()
        if 'escape' in keys:
            check_escape()

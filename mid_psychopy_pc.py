...
def run_trial(trial_idx, block_idx, cond_label):
    ...
    # win.flip()
    # onset = core.getTime()
    # core.wait(target_ms / 1000.0)  # target visibility window

    # # Target off
    # fixation.draw()
    # win.flip()

    # # Collect key if pressed during target window
    # keys = kb.getKeys(CONFIG["resp_keys"], waitRelease=False, clear=False)
    # if len(keys) > 0:
    #     # take the first key that occurred during or after onset; check time
    #     first = min(keys, key=lambda k: k.rt)
    #     # psychopy Keyboard.rt is time since last kb.clock.reset(); safer to compute relative:
    #     # Use event.getKeys alternative for broader compatibility
    #     # Here we approximate with first.rt if set; else compute delta
    #     try:
    #         rt = first.rt * 1000.0
    #         keyname = first.name
    #     except Exception:
...
if CONFIG["practice_trials"] > 0:
    ...
    txt.draw(); win.flip(); event.waitKeys()
...
for b in range(1, CONFIG["n_blocks"]+1):
    ...
    blk.draw(); win.flip(); event.waitKeys()
